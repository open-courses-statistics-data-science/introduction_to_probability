...
\logo{\includegraphics[width=.15\textwidth]{figures/msc-logo.png}}
...
\begin{document}
	...
	\begin{frame}
		...
		\setlength{\epigraphwidth}{0.7\textwidth} \epigraph{There are known knowns. These are things we know that we know. There are known unknowns. That is to say, there are things that we know we don't know. But there are also unknown unknowns. There are things we don't know we don't know.}{Donald Rumsfeld, 2002}
	...
	Consider a fair six-sided die with numbers 1-6 on each face that is thrown once.
		...
		\begin{enumerate}
			...
			\item \textit{Advanced}: if six dice are thrown, what's the probability that one of each of the numbers is obtained? If you like, use R \\to check your result.
		\end{enumerate}
	...
	Question: given that a woman tests positive, what is \\the probability that she has breast cancer?
	...
	\begin{frame}
		...
		\vspace{-.5cm} We attach a \textit{probability density} to each possible value the random variable can take. These densities represent a local \textit{concentration} of probability.
